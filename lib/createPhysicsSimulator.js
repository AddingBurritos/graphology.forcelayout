...
export default function createPhysicsSimulator(graph, settings) {
  ...
  const publicApi = {
    ...
    getSpring: function (edgeId) {
      if (!graph.hasEdge(edgeId)) {
        throw new Error("cannot getSpring from nonexistent edge");
      }
    ...
    getBestNewBodyPosition: function (nodeId) {
      let neighborsHaveBodies = false;
      if (graph.hasNode(nodeId)) {
        neighborsHaveBodies = graph.everyNeighbor(nodeId, (neighbor, attributes) => {
          if (!attributes[settings.body]) {
            return false;
          }
          return true;
        });
      ...
      let newPos;
      if (neighborsHaveBodies) {
        newPos = bounds.getBestNewPosition(nodeId);
      } else {
        newPos = bounds.getBestNewPosition();
      }
      return newPos;
    },
    ...
    isNodeOriginallyPinned: function(nodeId) {
      if (!graph.hasNode(nodeId)) {
        throw new Error("node does not exist");
      }
      if (graph.hasNodeAttribute(nodeId, settings.isPinned)) {
        return graph.getNodeAttribute(nodeId, settings.isPinned);
      }
      return false;
    },
  };
  ...
  function getBody(nodeId) {
    if (!graph.hasNode(nodeId)) {
      throw new Error("cannot getBody from nonexistent node");
    }
