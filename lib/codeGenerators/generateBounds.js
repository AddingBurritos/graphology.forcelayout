...
export function generateBoundsFunctionBody(dimension) {
  ...
  return {
    ...
    getBestNewPosition: function (nodeId) {
      let ${pattern('base_{var} = 0', {join: ', '})};
      if (graph.hasNode(nodeId)) {
        const degree = graph.degree(nodeId);
        if (degree) {
          graph.forEachNeighbor(nodeId, (neighbor, attributes) => {
            const neighborPos = attributes[settings.body].pos;
            ${pattern('base_{var} += neighborPos.{var};', {indent: 12})}
          });
          
          ${pattern('base_{var} /= degree;', {indent: 10})}
        } else {
          ${pattern('base_{var} = (boundingBox.min_{var} + boundingBox.max_{var}) / 2;', {indent: 10})}
        }
      } else {
